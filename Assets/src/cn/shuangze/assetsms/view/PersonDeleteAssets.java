 ...
 * 人员信息删除界面
 */
public class PersonDeleteAssets extends JFrame implements ActionListener, ListSelectionListener {
    ...
    public PersonDeleteAssets() {
        personTypeDao = new PersonTypeDaoJDBCImpl();
        //设置框架的大小
        try {
            init();
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "窗口初始化错误，错误原因：" + e.getMessage(), "人员信息删除错误", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (rowcount >= 0) {
            try {
                //删除
                int i = JOptionPane.showConfirmDialog(null, "确定要删除吗?", "人员信息删除", JOptionPane.YES_NO_CANCEL_OPTION);//单击"确定"按钮,则返回值为0.
                if (i == 0) {
                    personTypeDao.delete(Integer.parseInt(jTextField1.getText()));
                    JOptionPane.showMessageDialog(null, "人员信息删除成功！", "人员信息删除", JOptionPane.INFORMATION_MESSAGE);
                }else{
                    JOptionPane.showMessageDialog(null, "人员信息未删除！", "人员信息删除", JOptionPane.INFORMATION_MESSAGE);
                }
            } catch (Exception ex) {
